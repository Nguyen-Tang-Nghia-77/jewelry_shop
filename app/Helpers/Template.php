...
class Template {
    ...
    public static function showItemHistory ($by, $time) {
        $xhtml = sprintf(
            ...
            <p class="mb-0 history-time"><i class="far fa-clock"></i> %s</p>', $by, date(Config::get('zvn.format.short_time'), strtotime($time)) );
        return $xhtml;
    }
    ...
    public static function showButtonAction ($controllerName, $id) {
        ...
        foreach ($listButtons as $btn) {
            ...
            $xhtml .= sprintf(
                '<a href="%s" type="button" class="rounded-circle btn btn-sm %s" data-toggle="tooltip" data-placement="top" data-original-title="%s">
                    <i class="fa %s"></i>
